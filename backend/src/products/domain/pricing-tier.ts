export class PricingTierLogic {
  static calculateUnitPrice(tiers: { minQuantity: number; maxQuantity: number | null; unitPrice: number }[], quantity: number): number {
    if (!tiers || tiers.length === 0) throw new Error('Pricing tiers not defined');
